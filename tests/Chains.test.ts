...
describe('kiemTra bat duoc loi', () => {
  ...
  it('hang khong ai lam ra', () => {
    const nha = docNha({
      nha: [
        { ten: 'a', hien: 'A', sprite: 'nha_nho_do', kieu: 'tieu_thu', so: 1, nhip: 1, vao: [{ hang: 'go', so: 1 }], ra: [] },
      ],
    });
  ...
  it('hang khong ai dung den', () => {
    const nha = docNha({
      nha: [{ ten: 'a', hien: 'A', sprite: 'nha_nho_do', kieu: 'san_xuat', so: 1, nhip: 1, vao: [], ra: [{ hang: 'go', so: 1 }] }],
    });
    const chuoi = docChuoi({ chuoi: [{ ten: 'c', hien: 'C', qua: ['a', 'a'] }] });
  ...
  it('nha quen ke vao chuoi nao', () => {
    const nha = docNha({
      nha: [
        { ten: 'a', hien: 'A', sprite: 'nha_nho_do', kieu: 'san_xuat', so: 1, nhip: 1, vao: [], ra: [{ hang: 'go', so: 1 }] },
        { ten: 'b', hien: 'B', sprite: 'nha_nho_do', kieu: 'tieu_thu', so: 1, nhip: 1, vao: [{ hang: 'go', so: 1 }], ra: [] },
      ],
    });
    const chuoi = docChuoi({ chuoi: [{ ten: 'c', hien: 'C', qua: ['a', 'a'] }] });
