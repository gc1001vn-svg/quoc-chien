...
| `linh_sung_1x_0.png` · `linh_sung_2x_0.png` · `linh_sung_2x_1.png` | Mẻ lính đời súng + hiện đại, Phase 10B. Người, ngựa, kiếm, động tác: Quaternius như `linh_co`, <https://quaternius.com>. **Icosa Gallery** (<https://icosa.gallery>, xem `docs/KHO_ICOSA.md`): xe tăng "Tank" <https://icosa.gallery/view/8LQYdoL4kmI> · đại bác "Rev war cannon" <https://icosa.gallery/view/7pfX_iKfJyH> · súng "low poly bolt rifle" <https://icosa.gallery/view/epemCvhKIb0> | Quaternius (Tomás Laulhé) · **Veeranjay Singh** (xe tăng) · **Michael Pretz** (đại bác) · **Hell Houndz Productions** (súng) | CC0 1.0 (Quaternius) · **CC-BY 3.0** (ba model Icosa) | 26/09/2026 |
| `linh_sung_1x.json` · `linh_sung_2x.json` | Toạ độ do `tools/nuong_sprite.mjs` sinh ra | Dự án | MIT | 26/09/2026 |
...
| KayKit Adventurers 2.0 · Character Animations · Fantasy Weapons — **không còn dùng** (mẻ `linh_co` dùng sáng 25/09, đổi sang Quaternius chiều cùng ngày) | Kay Lousberg | `npm run kho:lay kaykit/adventurers` … |
| **Universal Animation Library** — bản `Unreal-Godot/UAL1_Standard.glb`, khớp xương bộ quần áo Quaternius: đi, chém kiếm, đâm, trúng đòn, chết, ngồi ngựa (mẻ `linh_co`, thêm 25/09). Bản trong kho chung là bản Godot, tên xương khác, **không dùng được** | Quaternius | `node tools/tai_itch.mjs quaternius/universal-animation-library` |
| **Medieval Weapons** — giáo, kiếm, khiên, cung, mũi tên (mẻ `linh_co`, thêm 25/09), thư mục `medieval-weapons` | Quaternius | `npm run kho:lay quaternius/medieval-weapons` |
